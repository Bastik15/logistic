    ...
    <div class="main-wrap" style="min-width: 850px;">
        <div class="main_content">
            ...
            <div class="main_content" style="margin-top: 10%;">
                ...
                <form action="{{ route('order.storeOrder', $order->id) }}" method="POST">
                    @csrf
                    <table cellspacing='0'>
                        <tr>
                            <th>
                                    ...
                                    @foreach ($products as $product)
                                        @if (is_null($orderProducts->where('product_id', $product->id)->first()))
                                            <option value="{{ $product->id }}">
                                                {{ isset($product->name) ? $product->name : $product->product->name }}
                                            </option>
                                        @endif
                                    @endforeach
                                </select>
                            ...
                            <th>
                                <input value="{{ old('price') }}" class="@error('price') is-invalid @enderror"
                                    placeholder='Цена' type='number' name='price' required>
                            </th>
                        </tr>
                        <tr>
                            <th>
